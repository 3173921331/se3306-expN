...
SSR 说明：SSR 依赖 Node 服务持续运行，静态托管无法部署。因腾讯云 CloudBase 存在实名扣费风险，未提交云托管链接。本地运行方式：进入 `lab1-ssr` 目录，执行 `node server.js`，浏览器访问 `http://localhost:3000` 即可验证 SSR 效果。

Github仓库地址：https://github.com/3173921331/se3306-expN/tree/main/se3306-exp1
...
3. SSG：构建阶段用 Node 脚本生成静态 HTML，部署时使用静态文件服务器（如 serve、python http.server）直接返回预生成文件，运行时不执行渲染。
...
### （7）：什么条件下SSG是最优解？内容更新频繁时如何补救？
SSG 最适合**内容不常更新、对首屏速度和稳定性要求高、无需个性化**的场景，例如博客、文档站、产品营销页、静态活动页。这些页面构建后内容固定，访问时直接返回静态文件，速度最快、成本最低、SEO 友好。

如果内容更新频繁，SSG 的“构建后固定”就会成为短板。补救方案是引入 **ISR（增量静态再生成）**：设置 revalidate 时间，页面过期后后台重新生成，用户仍访问静态文件，同时内容能得到更新。也可采用“SSG + 前端异步拉取”的混合模式，静态骨架保证首屏，动态数据由客户端请求补充
